...
// We will load this from a generated JSON file
...
const GoogleReviews = () => {
  ...
  // Use data from the JSON file, or fallback to empty array if valid structure isn't there
  const reviews = reviewsData?.reviews || [];
  const totalRating = reviewsData?.rating || "4.5";
  const userRatingsTotal = reviewsData?.user_ratings_total || "101";
        ...
        <div className="bg-black text-white rounded-xl p-6 md:p-8 max-w-7xl mx-auto shadow-2xl overflow-hidden relative">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
            <div className="flex items-center gap-4">
              <span className="text-4xl md:text-5xl font-bold">{totalRating}</span>
              <div className="flex flex-col">
                ...
                <a href="#" className="text-sm underline hover:text-gray-300">{userRatingsTotal} reviews on Google</a>
              </div>
            </div>
            <Button className="bg-[#1a73e8] hover:bg-[#1557b0] text-white rounded-full px-6 py-2 font-medium" onClick={() => window.open("https://search.google.com/local/writereview?placeid=" + (reviewsData?.place_id || ""), "_blank")}>
              Review us on Google
            </Button>
          </div>
                  ...
                  <div key={index} className="flex-[0_0_280px] md:flex-[0_0_320px] min-w-0">
                    <div className="bg-white text-black p-5 rounded-xl h-full flex flex-col min-h-[220px]">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <div className="flex items-center gap-1">
                              <span className="font-bold text-sm line-clamp-1">{review.author_name}</span>
                              {/* Google API doesn't confirm verification like some widgets, but we assume high quality. */}
                      ...
                      {/* {review.text.length > 100 && (
                             <span className="text-sm text-gray-500 mt-auto inline-block">Read more on Google</span>
                        )} */}
                    </div>
                  </div>
                ))}
